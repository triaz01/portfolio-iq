...
def calculate_ips(time_horizon, liquidity, objective, crash_reaction, knowledge):
    ...
    """
    print(f"[IPS] IPS Inputs: Time={time_horizon}, Liquidity={liquidity}, Objective={objective}, Crash={crash_reaction}, Knowledge={knowledge}")
    ...
    print(f"[IPS] Ability Score: {ability_score}, Willingness Score: {willingness_score}")
    ...
    print(f"[IPS] Final Risk Profile: {risk_profile}")
...
def assess_alignment(actual_metrics, ips_targets, profile):
    ...
    if beta_min <= actual_beta <= beta_max:
        alignment_bullets.append(f"[OK] **Portfolio Beta ({actual_beta:.2f})** is within your target range of {beta_min:.2f} - {beta_max:.2f}")
    else:
        if actual_beta > beta_max:
            alignment_bullets.append(f"[WARN] **Portfolio Beta ({actual_beta:.2f})** exceeds your maximum target of {beta_max:.2f}. Consider reducing exposure to high-volatility growth stocks or adding defensive positions to align with your {profile} profile.")
        else:
            alignment_bullets.append(f"[WARN] **Portfolio Beta ({actual_beta:.2f})** is below your minimum target of {beta_min:.2f}. Consider adding growth-oriented stocks to increase market sensitivity for your {profile} objectives.")
    
    # Assess Volatility alignment
    if vol_min <= actual_volatility <= vol_max:
        alignment_bullets.append(f"[OK] **Annualized Volatility ({actual_volatility:.1%})** is within your target range of {vol_min:.1%} - {vol_max:.1%}")
    else:
        if actual_volatility > vol_max:
            alignment_bullets.append(f"[WARN] **Annualized Volatility ({actual_volatility:.1%})** exceeds your maximum target of {vol_max:.1%}. Consider diversifying across sectors, adding lower-volatility assets, or reducing concentration in high-beta stocks to match your {profile} risk tolerance.")
        else:
            alignment_bullets.append(f"[WARN] **Annualized Volatility ({actual_volatility:.1%})** is below your minimum target of {vol_min:.1%}. Your portfolio may be too conservative for your {profile} growth objectives. Consider adding growth stocks or reducing bond exposure.")
    
    # Assess Yield alignment
    if yield_min <= actual_yield <= yield_max:
        alignment_bullets.append(f"[OK] **Dividend Yield ({actual_yield:.2%})** is within your target range of {yield_min:.1%} - {yield_max:.1%}")
    else:
        if actual_yield < yield_min:
            alignment_bullets.append(f"[WARN] **Dividend Yield ({actual_yield:.2%})** is below your minimum target of {yield_min:.1%}. Consider adding dividend-paying stocks or income-focused assets to meet your {profile} income requirements.")
        else:
            alignment_bullets.append(f"[WARN] **Dividend Yield ({actual_yield:.2%})** exceeds your maximum target of {yield_max:.1%}. Consider reducing income-focused holdings to better align with your {profile} growth objectives.")
    
    # Overall alignment assessment
    aligned_count = sum(1 for bullet in alignment_bullets if bullet.startswith("[OK]"))
