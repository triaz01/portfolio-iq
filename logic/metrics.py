...
def generate_trading_signals(tickers, benchmark='SPY'):
    ...
    try:
        ...
        print(f"📈 Fetching 400 days of data for {len(tickers)} tickers + benchmark ({benchmark})")
        ...
        if benchmark_hist.empty or len(benchmark_hist) < 126:
            print(f"❌ Insufficient benchmark data for {benchmark}")
        ...
        print(f"✅ Benchmark data fetched: {len(benchmark_close)} days")
        ...
        for ticker in tickers:
            ...
            try:
                ...
                if hist.empty or len(hist) < 252:
                    print(f"  ⚠️ Insufficient data for {ticker} (need 252 days, got {len(hist)})")
                    results.append({
                        'Ticker': ticker,
                        'Momentum (%)': 'N/A',
                        'Trend vs 200DMA (%)': 'N/A',
                ...
                print(f"  📊 12-1 Momentum: {momentum*100:.2f}% (Score: {momentum_score})")
                ...
                print(f"  📈 200-Day Trend: {trend*100:.2f}% (Score: {trend_score})")
                ...
                if len(benchmark_aligned) >= 126:
                    ...
                    print(f"  💪 Relative Strength vs {benchmark}: {rel_strength*100:.2f}% (Score: {rel_strength_score})")
                else:
                    # Fallback if alignment fails
                    rel_strength = 0
                    rel_strength_score = 0
                    print(f"  ⚠️ Could not align benchmark dates, using neutral score")
                ...
                print(f"  🎯 Total Score: {total_score} → {recommendation}")
